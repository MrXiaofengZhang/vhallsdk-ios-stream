...
- (void)initViews
{
    ...
    _moviePlayer = [[VHallOpenMoviePlayer alloc]initWithDelegate:self];
...
- (IBAction)stopWatchBtnClick:(id)sender
{
    if (_isStart) {
        ...
        {
           _bufferCount = 0;
           [_moviePlayer startPlay:_roomId];

        }else if(_watchVideoType == kWatchVideoHLS)
        {
           //观看直播
           [_moviePlayer startPlay:_roomId  moviePlayer:self.hlsMoviePlayer];

        }else if(_watchVideoType == kWatchVideoPlayback){
           
           //观看回放地址
           [_moviePlayer startPlayback:_roomId  moviePlayer:self.hlsMoviePlayer];
